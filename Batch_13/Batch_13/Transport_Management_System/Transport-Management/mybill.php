<?php 
    ...
    $query= "SELECT booking.booking_id, booking.pic_date,booking.`dil_date`, booking.`destination`, booking.`veh_reg`, booking.`driverid`, tripcost.total_km,tripcost.fuel_cost, tripcost.extra_cost,tripcost.total_cost,tripcost.paid FROM `booking` LEFT JOIN `tripcost` ON booking.username=tripcost.username WHERE booking.username='$username' AND booking.booking_id=tripcost.booking_id;";
...
?>
                    <tr>
                        ...
                        <td><a href="truckprofile.php?truckid=<?php echo $row['veh_reg'] ?>"><?php echo $row['veh_reg'] ?></a></td>
